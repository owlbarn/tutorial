...
Before moving on, let's take a brief look at the symplectic solvers, which are a bit different than the native solvers we have seen so far.
...
In `Owl_ode` we have implemented several symplectic solvers that are based on different integration algorithms, such as the Leapfrog.
