...
Part of the data is listed here.
...
The visualisation result is shown here.
...
$$h_{\theta_0, \theta_1}(x_1) = \theta_0~ + \theta_1~x_1$$ 
...
$$J_{\theta_0, \theta_1}(\boldsymbol{x}, \boldsymbol{y}) = \frac{1}{2n}\sum_{i=1}^{n}(h_{\theta_1, \theta_0}(x_i) - y_i)^2$$ 
...
The cost function  `j` is implemented as below:
...
Here `x_data` and `y_data` are the two columns of data.
...
In the figure below we can see that cost function varies with parameters $$\theta_0$$ and $$\theta_1$$ with a bowl-like shape curve surface.
...
$$ \theta_j \leftarrow \theta_j - \alpha~\frac{\partial}{\partial \theta_j}~J_{\theta_0, \theta_1}(\boldsymbol{x}, \boldsymbol{y}), $$ 
where $$i$$ is 1 or 2.
...
$$ \theta_0 \leftarrow \theta_0 - \frac{\alpha}{n}\sum_{i=1}^{m} (h_{\theta_0, \theta_1}(x_i) - y_i)x_{i}^{(0)}, $$ 

and

$$ \theta_1 \leftarrow \theta_1 - \frac{\alpha}{n}\sum_{i=1}^{m} (h_{\theta_0, \theta_1}(x_i) - y_i)x_{i}^{(1)}.$$ 
...
The resulting figure shows a line that aligns with input data quite nicely.
...
We can use the gradient descent optimisation methods introduced in the Optimisation chapter, and apply them directly on the cost function.
...
$$h_({\theta_0, \theta_1, \theta_2, \theta_3, \ldots})(x_1, x_2, x_3, \ldots) = \theta_0~ + \theta_1~x_1 + \theta_2~x_2 + \theta_3~x_3 \ldots $$ 
...
$$h_{\boldsymbol{\theta}}(X^{(i)}) = \boldsymbol{\theta}~X^{(i)},$$ 
...
$$ J_{(\boldsymbol{\theta}}(X, \boldsymbol{y}) = \frac{1}{2n}\sum_{i=1}^{n}(\theta~X^{(i)} - y^{(i)})^2,$$ 
...
Hmm, it might be right, but something about this model feels wrong. If you apply any line of data to this model, the prediction result deviates too much from the true `y` value.
...
Another benefit of performing data normalisation is to accelerate gradient descent. The illustration below shows the point.
...
The relationship basically follows a convex curve.
...
$$ h_{\boldsymbol{\theta}}(x) = \theta_0 + \theta_1~x + \theta_2~x^2 + \theta_3~x^3 \ldots $$ 
...
This model could be visualised as above.
...
$$J_{\boldsymbol{\theta}}(\boldsymbol{x}, \boldsymbol{y})=\frac{1}{2n}\left[ \sum_{i=1}{n}(h_{\theta}(x^{(i)} - y^{(i)}))^2 + \lambda\sum_{j=1}^{m}\theta_j^2 \right].$$ 
...
By using this regularised version of polynomial regression, we can have an updated model as shown in the figure below.
...
This function projects value within the range of [0, 1].
Applying this function on the returned value of a regression, we can get a model that returns value within [0, 1].

$$h_{\boldsymbol{\theta}}(\boldsymbol{x}) = \sigma(\boldsymbol{\theta}^T~\boldsymbol{x}) = \frac{1}{1 + e^{-\boldsymbol{\theta}^T~\boldsymbol{x}}}.$$ 
...
The cost function of linear regression indicates the sum of squared distances between the data and the model line.
...
$$J_{\boldsymbol{\theta}}(\boldsymbol{x}, \boldsymbol{y}) = \frac{1}{m}\sum_{i=1}^{m}g(h(x^{(i)})-y^{(i)}),$$ 

where the function $$g$$ is defined as:

$$g(h_{\boldsymbol{\theta}}(x), y) = -log(h_{\boldsymbol{\theta}}(x)), \textrm{if }~y = 1, $$  or

$$g(h_{\boldsymbol{\theta}}(x), y) = -log(1 - h_{\boldsymbol{\theta}}(x)), \textrm{if }~y = 0.$$ 

Both forms of function $$g()$$ capture the same idea.
Since the $$h$$ function is in the range [0, 1], the range of $$g$$ is [0, $$\infty$$].
When the value of $$h(x)$$ and $$y$$ are close, then the item within the summation in $$g(h(x)) - y$$ will be close to 0;
...
$$J_{\boldsymbol{\theta}}(\boldsymbol{x}, \boldsymbol{y}) = \frac{1}{m}\sum_{i=1}^{m}(y^{(i)}\log(h(x^{(i)})) + (1-y^{(i)})\log(1-h(x^{(i)})))$$ 

The next step is to find the partial derivative of this cost function and then iteratively minimise it to find suitable parameters $$\Theta$$.
It turns out that the partial derivative of this cost function is similar as that in linear regression:

$$\frac{\partial J_{\boldsymbol{\theta}}(\boldsymbol{x}, \boldsymbol{y})}{\partial \theta_j} = \frac{1}{m}\sum_{i=1}^{m}(\sigma_{\boldsymbol{\theta}}(x^{(i)}) - y^{(i)})^2~x_j^{(i)}$$ 

This solution benefits from the fact that the sigmoid function has a simple derivative: $$\sigma^{'}~(x) = \sigma(x)~(1 - \sigma(x))$$.

With this derivative at hand, the rest is similar to what we have done with linear regression: repeat this gradient descent step until it converges.
...
$$h(x_0, x_1) = \sigma~(16~x_0 + 12~x_1 + 20).$$ 
...
Any $$x_0$$, $$x_1$$ that makes the $$h(x_0, x_1) > 0$$ is taken as positive, otherwise it's negative.
...
Again, let's start with the objective cost function. It turns out that the cost function of the SVM is very similar to that of logistic regression, with some modifications:

$$J_{\boldsymbol{\theta}}(\boldsymbol{x}, \boldsymbol{y}) = \frac{1}{m}\sum_{i=1}^{m}(y^{(i)}g_0(\boldsymbol{\theta}^T~x^{(i)}) + (1-y^{(i)})g_1(\boldsymbol{\theta}^T~x^{(i)})) + \frac{1}{2}\sum_{j=1}^m\theta_j^2$$ 
...
Also, another difference is that a regularisation item is added to the cost function.
...
One example is shown in the figure above.
...
$$f_{\boldsymbol{\theta}}(\boldsymbol{x}) = \theta_0 + \theta_1x_1 + \theta_2x_2 + \theta_3x_1x_2 + \theta_4x_1^2 + \theta_5x_2^2 + \ldots$$ 
...
With the trained parameters $$\theta$$, if the equation is larger than zero, then the inference result is positive, otherwise it's negative.
...
$$d_i = \exp(-\frac{\sum_{j=1}^n(x_j - p_k^{(j)})}{2\sigma^2}).$$ 
...
For example, in the figure we choose only three reference points: $$p_0$$, $$p_1$$, and $$p_2$$.
...
The Gaussian kernel we have just used is a commonly used one, but other kernel functions can also be used, such as the polynomial kernel, the Laplacian kernel, etc.
...
We can visualise this boundary line by setting $$f(x)=0$$. Here the `y` axis is $$x_2$$, and `x` axis is $$x_1$$.  
...
We have already mentioned the issue of feature selection in the previous sections.
...
Regularisation is one way to downplay these features, and in this section we briefly introduce another commonly used technique: *feature selection*.
